...
function PrimaryMotivationUniversity({}: Props) {
  const {
    setFormData,
    nextPage,
    previousPage,
  ...
  const [motivationData, setMotivationData] = useState(
    defaultData.primaryMotivationUniversity || primaryMotivationData
  );
  ...
  async function onNext() {
    setIsLoading(true);
    setFormData({ primaryMotivationUniversity: motivationData });
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="space-y-8">
        <Droppable
          droppableId="motivation-university"
          key="motivation-university"
          direction="vertical"
        >
          {(provided) => (
            <div
...
export default PrimaryMotivationUniversity;
